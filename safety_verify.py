...
class Verify:
    ...
    def gen_primitive_lib(self, velocities, steers, dt=1):
        ...
        for i in range(controls.shape[0]):
            ...
            if steer != 0:
                velocity = 0
            lib[:,:,i] = SE2Transformation(np.array([[1,0]]).T*velocity*dt,steer)
    ...
    def verify_safety(self,infos,T,action,threshold=.3,verbose=False):
        ...
        if infos:
            ...
            currentpose = SE2Transformation(infos[0]['top_down_map']['agent_map_coord'], infos[0]['top_down_map']['agent_angle'])
            # print(currentpose)
            if action == 1:
            ...
            for t in range(T):
                ...
                for i in range(open_set.shape[2]):
                    for j in range(self.primitive_lib.shape[2]):
                        new_pose = np.dot(open_set[:,:,i],self.primitive_lib[:,:,j])
